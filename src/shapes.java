...
Comments:
  interfaceShape doesn't need to implement toString, since that's on Object
  don't hard-code the image paths
  Your copy/paste hurt you. Here's an example:
			for (int i = 0; i < circleList.size(); i++) {
				if (check == firstW.compareTo(squareList.get(i).toString())) {
    you are checking circleList.size, so squareList.get(i) is out of range.
  You don't need MouseMotionListener, since all you implement is mousePressed.
*/
